...
public class TrafficSignal extends IntersectionControl implements Signalized
{
    ...
    public Phase getCurrentPhase() {
        return this.phases.get(curr_idx);
    }
...
//        }
    ...
    */
    ...
    // make method to return times in phase
    public boolean isGreen(double t, Link i, Link j){
        ...
        for(Phase p : phases){
            if(time_rem < p.getDuration()){

                if(time_rem < p.getGreenTime()){
                    for(Turn allow : p.getAllowed()){
                        if(allow.i == i && allow.j == j){
                            return true;
                        }
                    }
                    return false;
    ...
    public double greenTimeRemaining(double t, Link i, Link j) {
        Phase phase = this.phases.get(this.curr_idx);

        if (isGreen(t, i, j)) { // in the green phase
            return phase.getRemainingTime(t% (gettotcycle()) );
        }

        // need way to reliably ensure that this is the correct ahead phase
        else { // get the green phase length
            return ( this.phases.get(1).getDuration() + phase.getRemainingTime(t% gettotcycle() ) );
        }
    }

    public double gettotcycle () {
        return this.phases.get(0).getDuration() + this.phases.get(1).getDuration();
    }

    // returns the total green time in a full cycle
    public double getgreenduration() {
        return this.phases.get(1).getDuration();
    }


    // returns the total red time in a full cycle
    public double getredduration() {
        return this.phases.get(0).getDuration();
    }

    public boolean isRed() {
//        System.out.println( "traffic signal" + this.getCurrentPhase().getGreenTime(Simulator.time));
        if (this.getCurrentPhase().getGreenTime(Simulator.time % gettotcycle()) == 0) {
            return true;
        }
        else {
            return false;
    ...
    // returns the duration of a full green phase
    public double greenphaseduration(){
        return this.phases.get(1).getDuration();
    }

    public double getGreenLength() {
        Phase phase = this.phases.get(this.curr_idx);
        return phase.getDuration() - phase.getRedTime();
    }
}
