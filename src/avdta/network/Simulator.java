...
/**
    ...
    // number of steps performed in each link
    public static final int NUM_TIMESTEPS = 5;
            //(int)Math.ceil(Simulator.duration / Network.dt)+1;
    ...
    public void nextTimestep() throws IOException
    {
        //System.out.println(time+"\t"+getNumVehiclesInSystem()+"\t"+occupancy_900);
            // push vehicles onto centroid connectors at departure time
        System.out.println("timestep #"+indexTime(time)+" starting");
        addVehicles();
        propagateFlow();
        System.out.println("timestep #"+indexTime(time)+" ended");
    }
    ...
    public void addVehicles()
    {
        ...
        while(veh_idx < vehicles.size())
        {
            ...
            // added - 10 for correct timing
            else if(v.getDepTime() - 10 <= Simulator.time)
            {
                v.entered();
    ...
    public void propagateFlow()
    {
        ...
        for(Node n : nodes)
        {
//            System.out.println("n.preparing "+n.getId());
            n.prepare();
        }
        
        for(Link l : links)
        {
//            System.out.println("l.preparing "+l.getId());
            l.prepare();
        }
        ...
        for(Link l : links)
        {
//            System.out.println("l.stepping "+l.getId());
            l.step();
        }

        for(Node n : nodes)
        {
//            System.out.println("n.stepping "+n.getId());
            exit_count += n.step();
        }

        for(Link l : links)
        {
//            System.out.println("~~~ updating link "+l+" ~~~");
            l.update();
        }
        System.out.println("timestep #"+indexTime(time)+" concluded");
//        System.out.println();
    }
