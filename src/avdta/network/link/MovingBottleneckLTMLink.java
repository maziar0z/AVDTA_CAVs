...
//            System.out.println("huu " + signal.getgreenduration() + signal.greenTimeRemaining(Simulator.time, trajectory.getVehicle().getCurrLink(), trajectory.getVehicle().getNextLink()) );
            m = (int) (-1 * Math.min(signal.getgreenduration(), signal.greenTimeRemaining(Simulator.time, trajectory.getVehicle().getCurrLink(), trajectory.getVehicle().getNextLink())) + 2 ) ; // min of green phase and remained time, +2 added as buffer
        }
        else {
            m = (int) - signal.getCurrentPhase().getGreenTime() + 2; // added + 2 to have a buffer
        }

        if (signal == null) {
            return this.calcArrivalTime(trajectory);
        }
        System.out.println("effective time " + (signal.greenTimeRemaining(Simulator.time, trajectory.getVehicle().getCurrLink(), trajectory.getVehicle().getNextLink()) + this.calcNumCycles(trajectory) * signal.gettotcycle() + m));
        ...
        return signal.greenTimeRemaining(Simulator.time, trajectory.getVehicle().getCurrLink(), trajectory.getVehicle().getNextLink()) + this.calcNumCycles(trajectory) * signal.gettotcycle() + m;
    }
...
//                "\n\teq2 = " + eq2 +
//                "\n\teq3 " + eq3);
        System.out.println("time " + Simulator.time + " final Sending flow" + Math.min(eq1, eq2));
        }
